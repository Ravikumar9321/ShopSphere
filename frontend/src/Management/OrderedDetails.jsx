...
function OrderedDetails() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://localhost:8080/api/order/sessionId/${sessionId}`);
  ...
  const confirmPayment = async (id) => {
    setLoading(true);
    try {
      await axios.post(`http://localhost:8080/api/payment/order/${id}`,{});
      alert("✅ Payment done");
      loadOrders();
    } catch (error) {
      ...
      <div style={styles.container}>
        ...
        {orderDetails.length === 0 ? (
          <div style={styles.emptyState}>
            ...
            <button style={styles.addOrderBtn} onClick={() => navigate(`/cartitems/${sessionId}`)}>
              Continue Shopping →
            </button>
          </div>
        ) : (
