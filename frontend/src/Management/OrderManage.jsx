...
function OrderManage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const cartRes = await axios.get(`http://localhost:8080/api/cartitem/sessionId/${sessionId}`);
  ...
  const createOrder = useCallback(async (e) => {
    try {
      const response = await axios.post(`http://localhost:8080/api/order/${sessionId}`,{});
      const order = response.data.data || response.data;
      setOrder(order);
      setOriginalCartTotal(order.totalAmount); 
    } catch (err) {
      const message = err.response?.data?.message || "Order creation failed.";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const completePayment = useCallback(async () => {
    ...
    setError(null);
    try {
      await axios.post(`http://localhost:8080/api/payment/order/${order.id}`, {});
      setOrder(null);
      setCartItems([]);
    } catch (err) {
      setError( err.response?.data?.message);
    ...
    <div style={styles.main}>
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.sessionInfo}>Session: <code style={styles.code}>{sessionId}</code></p>
        </div>

        {/* Cart Summary */}
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>🛒 Cart Summary ({cartItems.length} items)</h2>
              ...
              cartItems.map((item) => (
                <div key={item.id} style={styles.cartItem}>
                  <div style={styles.itemDetails}>
                    ...
                    <p style={styles.itemQty}>Qty: {item.quantity} × ₹{item.product?.price}</p>
                  </div>
          ...
          {cartItems.length > 0 && !order &&  (
            <>
              ...
              <button 
                style={loading ? styles.primaryBtnDisabled : styles.primaryBtn} 
                onClick={createOrder}
                disabled={loading}
              >
        ...
        {!order && cartItems.length === 0  && originalCartTotal > 0 && (
          <div style={styles.successSection}>
            <div style={styles.successMessage}>
              ✅ Order placed and payment completed successfully! Total: ₹{originalCartTotal.toLocaleString()}
            </div>
          </div>
        )}
        ...
        <div style={styles.footer}>
          <button style={styles.backBtn} onClick={() => navigate(`/cartitems/${sessionId}`)}>
            ← Back to Cart
          </button>
